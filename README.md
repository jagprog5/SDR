...
```mermaid
flowchart TB
    SDR["SDR\nHas a container_t of SDR_elem_t elements"]
    SDR_elem_t["SDR_elem_t\nHas an id and (optionally) data"]
    ...
    FloatData["FloatData\nA float"]
    id_t["id_t\nAn integral type which uniquely identifies an SDR_elem_t in an SDR."]
    data_t["data_t\nA payload associated with the id."]

    SDR--template--->SDR_elem_t
    ...
    SDR_elem_t--template-->id_t
    SDR_elem_t--template--->data_t
...
## SDR_elem_t with Data

SDRs are composed of SDR_elem_t elements. By default, an SDR_elem_t has an `int` identifier and an `EmptyData` data.  
...
Data can define a "relevance". If the SDR_elem_t's data is not relevant, then it is not placed in the result.  
...
int main() {
    ...
    using UnitSDR_elem_t = SDR_elem_t<int, UnitData>;
    using UnitSDR = SDR<UnitSDR_elem_t>;

    auto r0 = UnitSDR{UnitSDR_elem_t(0, 0.5)}.ande(UnitSDR{UnitSDR_elem_t(0, 0.5)});
    ...
    auto r1 = UnitSDR{UnitSDR_elem_t(0, 0.25)}.ande(UnitSDR{UnitSDR_elem_t(0, 0.25)});
    std::cout << r1 << std::endl; // empty
    int r1_size = UnitSDR{UnitSDR_elem_t(0, 0.25)}.ands(UnitSDR{UnitSDR_elem_t(0, 0.25)});
    std::cout << r1_size << std::endl;  // 0

    // data types are compatible with other data types
    // EmptyData converts to UnitData(1)
    auto r2 = UnitSDR{UnitSDR_elem_t(0, 0.5)}.ande(SDR{SDR_elem_t(0)});
